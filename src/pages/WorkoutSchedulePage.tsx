...
export function WorkoutSchedulePage() {
  const { workouts, dispatch } = useAppState();
  ...
  const [rescheduleDate, setRescheduleDate] = useState('2026-07-25');
  const [createName, setCreateName] = useState('Heavy Legs');
  const [createDate, setCreateDate] = useState('2026-07-23');
  ...
  const schedule = useMemo(() => [...workouts].sort((a, b) => a.date.localeCompare(b.date)), [workouts]);
  const selectedItems = schedule.filter((workout) => workout.date === selectedDate);
  const unresolved = schedule.find((workout) => workout.id === 'plan-upper-0721-unresolved' && workout.status === 'planned');
  ...
  const startLate = (workoutId: string) => {
    dispatch({ type: 'start-planned-workout', workoutId });
  ...
  return (
    <div className="space-y-6 animate-rise">
      <header className="flex items-end justify-between gap-4">
        ...
        <Button variant="primary" icon={<Plus size={18} />} onClick={() => { setCreateDate(selectedDate); setCreateOpen(true); }}>Plan workout</Button>
      </header>
      ...
      <section className="lg:hidden">
        ...
        <div className="mt-2"><h2 className="text-lg font-bold">{formatDate(selectedDate, { weekday: 'long', day: 'numeric', month: 'long' })}</h2><div className="mt-3 space-y-3">{selectedItems.length ? selectedItems.map((workout) => <WorkoutScheduleCard key={workout.id} workout={workout} onOpen={openWorkout} />) : <EmptyDay onCreate={() => { setCreateDate(selectedDate); setCreateOpen(true); }} />}</div></div>
      </section>

      <section className="hidden lg:block">
        <div className="grid grid-cols-7 gap-2">
          {weekDates.map((day) => <div key={day.iso} className="min-w-0"><div className={`mb-2 rounded-[12px] px-2 py-3 text-center ${day.iso === '2026-07-23' ? 'bg-[var(--accent-soft)] text-[var(--accent)]' : 'text-[var(--text-muted)]'}`}><div className="text-xs font-bold uppercase tracking-[.1em]">{day.day}</div><div className="metric mt-1 text-xl font-black">{day.date}</div></div><div className="space-y-2">{schedule.filter((workout) => workout.date === day.iso).map((workout) => <WorkoutScheduleCard key={workout.id} workout={workout} onOpen={openWorkout} compact />)}<button className="grid min-h-16 w-full place-items-center rounded-[14px] border border-dashed border-[var(--border)] text-[var(--text-faint)] hover:border-[var(--border-strong)] hover:text-[var(--text-muted)]" onClick={() => { setSelectedDate(day.iso); setCreateDate(day.iso); setCreateOpen(true); }} aria-label={`Add workout on ${day.day}`}><Plus size={18} /></button></div></div>)}
        </div>
      </section>
      ...
      <Sheet open={Boolean(selectedWorkout)} onOpenChange={(open) => !open && setSelectedWorkout(null)} title={selectedWorkout?.name ?? 'Workout'} description={selectedWorkout?.recurrence ? `This Workout repeats weekly${selectedWorkout.recurrenceEndDate ? ` through ${formatDate(selectedWorkout.recurrenceEndDate, { day: 'numeric', month: 'short', year: 'numeric' })}` : ''}.` : 'Edit this planned occurrence.'}>
        {selectedWorkout && <div className="space-y-4">
          ...
          <Button full onClick={() => startLate(selectedWorkout.id)} icon={<Check size={17} />}>Complete late</Button>
      ...
      <Sheet open={createOpen} onOpenChange={setCreateOpen} title="Plan a workout" description="Create one occurrence or make it recur weekly.">
        <div className="space-y-4">
          <label className="block"><span className="mb-2 block text-xs font-bold uppercase tracking-[.1em] text-[var(--text-muted)]">Workout</span><select value={createName} onChange={(event) => setCreateName(event.target.value)} className="min-h-12 w-full rounded-[var(--radius-md)] border border-[var(--border)] bg-[var(--surface)] px-3"><option>Heavy Legs</option><option>Upper Strength B</option><option>Core + Carry Rounds</option><option>Long Run</option></select></label>
          <label className="block"><span className="mb-2 block text-xs font-bold uppercase tracking-[.1em] text-[var(--text-muted)]">Date</span><input type="date" value={createDate} onChange={(event) => setCreateDate(event.target.value)} className="min-h-12 w-full rounded-[var(--radius-md)] border border-[var(--border)] bg-[var(--surface)] px-3" /></label>
          <label className="flex min-h-12 items-center justify-between rounded-[var(--radius-md)] border border-[var(--border)] bg-[var(--surface)] px-3"><span className="flex items-center gap-2 font-semibold"><Repeat2 size={17} /> Repeat weekly for one year</span><input type="checkbox" checked={createRecurring} onChange={(event) => setCreateRecurring(event.target.checked)} className="size-5 accent-[var(--accent)]" /></label>
          <Button variant="primary" full onClick={() => { dispatch({ type: 'add-planned-workout', name: createName, date: createDate, recurrence: createRecurring ? 'weekly' : undefined }); setCreateOpen(false); }}>Add to Workout Schedule</Button>
        </div>
      </Sheet>
    </div>
  );
...
function WorkoutScheduleCard({ workout, onOpen, compact = false }: { workout: Workout; onOpen: (workout: Workout) => void; compact?: boolean }) {
  const unresolved = workout.status === 'planned' && workout.date < '2026-07-23';
  return (
