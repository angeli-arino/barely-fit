...
export function HistoryPage() {
  ...
  const completed = workouts.filter((workout) => workout.status === 'completed').sort((a, b) => b.date.localeCompare(a.date));
        ...
        {visible.map((workout) => {
          ...
          return (
            <Link key={workout.id} to={`/history/${workout.id}`} className="block rounded-[var(--radius-lg)] border border-[var(--border)] bg-[var(--surface)] p-4 shadow-[var(--shadow-1)] transition hover:border-[var(--border-strong)] hover:bg-[var(--surface-strong)] sm:p-5">
              ...
              <div className="mt-4 grid grid-cols-2 gap-2 border-t border-[var(--border)] pt-4 text-sm"><div className="flex items-center gap-2 text-[var(--text-muted)]"><TrendingUp size={16} /><span>{volume ? `${volume.toLocaleString()} kg volume` : 'Cardio + bodyweight'}</span></div><div className="flex items-center justify-end gap-2 text-[var(--text-muted)]"><CalendarCheck size={16} /><span>Synced</span></div></div>
            </Link>
          );
        })}
      </div>
